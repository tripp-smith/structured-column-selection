...
def SelectedGram {k n : ℕ}
    (A : Matrix (Fin k) (Fin n) ℝ) (J : Fin k → Fin n) : Matrix (Fin k) (Fin k) ℝ :=
  SelectedSquare A J * (SelectedSquare A J)ᵀ
...
theorem selectedGram_eq {k n : ℕ}
    (A : Matrix (Fin k) (Fin n) ℝ) (J : Fin k → Fin n) :
    SelectedGram A J = SelectedSquare A J * (SelectedSquare A J)ᵀ := rfl
